...
Live environments on all paid plans have multiple containers that can contain logs.  For Personal and Professinal plans, logs could be on a failover container in addition to the main, active container while Business and Elite plans have more than one active container in the Live *and* Test environments. In order to download the logs from each application container, use the following shell script:
